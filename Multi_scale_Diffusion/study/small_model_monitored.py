...
class Node2Vec(torch.nn.Module):
    ...
    def get_embedding(self, X1, X2):
        monitor.start_timer('embeddings')
        result = normalize( self.w1(X1) + self.w2(X2) + self.iden)
        monitor.end_timer()
        return result

    def update_B(self, X1, X2, unique=None):
        monitor.start_timer('encoder')
    ...
    def pos_sample(self, batch):
        monitor.start_timer('random_walk')
    ...
    def loss(self, pos_rw, neg_rw, mapping=None):
        ...
        result = -1*torch.mean(torch.exp(pos_loss-neg_loss))
        monitor.end_timer()
        return result
