...
def compute_adaptive_diffusion_v2(graph, x, max_hops=4, method="ppr", normalize_output=True):
    ...
    # PubMed-specific parameters (large graph, lower-dim features)
    if method == "ppr":
        alpha_values = [0.3, 0.3, 0.3, 0.1][:max_hops]
        base_alpha = 0.25  # Higher alpha for large graphs
    else:  # heat
        alpha_values = [0.35, 0.35, 0.25, 0.05][:max_hops]
        base_alpha = 0.25
...
def main():
    ...
    parser.add_argument('--save_embedding', type=str, default="embedding_pubmed.pt")
