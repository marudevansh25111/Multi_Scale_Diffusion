...
class clustering_metrics():
    ...
    def clusteringAcc(self):
        ...
        #numclass1 = len(l1)
        numclass1 = max(self.true_label)+1
        l1 = list([i for i in range(numclass1)])
        l2 = list([i for i in range(numclass1)])
    ...
    def evaluationClusterModelFromLabel(self, tqdm):
        ...
        #print("CALCULATING ADJSCORE")
        adjscore = metrics.adjusted_rand_score(self.true_label, self.pred_label)
        cs = metrics.completeness_score(self.true_label, self.pred_label)
        #y = self.clusteringAcc()
        #print(y)
        #exit()
